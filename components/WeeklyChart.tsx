...
export const WeeklyChart: React.FC<Props> = ({ data, title = "ANÁLISE SEMANAL" }) => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900/60 backdrop-blur-sm p-4 sm:p-10 rounded-2xl sm:rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 h-[280px] sm:h-[450px] shadow-2xl flex flex-col">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-10 gap-2">
        <h3 className="text-[9px] sm:text-[11px] font-black dark:text-zinc-100 text-zinc-800 uppercase tracking-[0.2em] sm:tracking-[0.3em] italic">{title}</h3>
      ...
      <div className="flex-1 w-full text-[8px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 0, right: 0, left: isSmall ? -30 : -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(120,120,120,0.05)" />
            <XAxis 
              dataKey="day" 
              axisLine={false} 
              tickLine={false} 
              tick={{ fill: '#71717a', fontSize: isSmall ? 7 : 8, fontWeight: 900 }} 
            />
            <YAxis 
              axisLine={false} 
              tickLine={false} 
